...
template <>
bool DataRef<base::Account>::LoadFromCsv(std::string_view filename) {
...
template <>
bool DataRef<base::Underlying>::LoadFromCsv(std::string_view filename) {
...
template <>
bool DataRef<base::Instrument>::LoadFromCsv(std::string_view filename) {
...
template <>
bool DataRef<base::CalendarDate>::LoadFromCsv(std::string_view filename) {
...
bool Context::Init(std::string_view data_folder) {
  ...
  if (!cal_ref_.LoadFromCsv((folder / "calendar.csv").string())) {
    std::cerr << "Failed to load calendar from csv file: " << folder
              << std::endl;
    return false;
  }

  if (!acc_ref_.LoadFromCsv((folder / "account.csv").string())) {
    std::cerr << "Failed to load account from csv file: " << folder
              << std::endl;
    return false;
  }
  if (!uly_ref_.LoadFromCsv((folder / "underlying.csv").string())) {
    std::cerr << "Failed to load underlying from csv file: " << folder
              << std::endl;
    return false;
  }
  if (!ins_ref_.LoadFromCsv((folder / "instrument.csv").string())) {
    std::cerr << "Failed to load instrument from csv file: " << folder
              << std::endl;
    return false;
  }
  for (auto i = 0; i < ins_ref_.Size(); ++i) {
    auto uly_id = uly_ref_.GetID(ins_ref_.Get(i).underlying_);
    ins_ref_.vec_[i].underlying_id_ = uly_id;
  }

  st_buf_.Resize(ins_ref_.Size());
  bar_buf_.Resize(ins_ref_.Size());
  depth_buf_.Resize(ins_ref_.Size());
  bal_buf_.Resize(acc_ref_.Size());
  return true;
}
